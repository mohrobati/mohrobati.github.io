$("#welcomeLogo").hover(
    function () {
        ...
        $('.dark-overlay').css('background-color', "rgba(0,0,0,0)");
    }
);

$("#welcomeLogo").click(
    function () {
        var curr = $('.dark-overlay').css('background-color')
        if (curr[curr.length - 2] === '0')
            $('.dark-overlay').css('background-color', "rgba(0,0,0,0.7)");
        else
            $('.dark-overlay').css('background-color', "rgba(0,0,0,0)");
    }
...
function activateCurrentSection() {
    ...
    if (lastSectionTooSmall && lastSectionInView) {
        ...
        sections.each(function () {
            var top = $(this).offset().top; // get the top & bottom position of the section
